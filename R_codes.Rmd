...
plot(ACF(Model.1, maxLag=10), alpha = .01)
```
...
```{r, eval=FALSE}
Model.6 <- update(Model.4, weights = varIdent(form = ~ 1 | design_type))
```
...
```{r}
Model.6.RandomEffect.se <- sqrt(diag(varcomp_vcov(Model.6)))
Model.6.RandomEffect.se
```

```{r}
M4_M6 <- anova(Model.4, Model.6)
M4_M6
```
