...
wp_data_dummy %<>% select(authorYear, study, designType, multiBase, multiProbe, cluster, case, phase, A, B, M, session, outcome, complexityMeasure, singleType, mixedType, generalization)
...
Model.4 <- lme(outcome ~
                 time_A + level_AB + trend_AB + level_BM + trend_BM + 
                 mixedType + time_A:mixedType + level_AB:mixedType + 
                 trend_AB:mixedType + level_BM:mixedType + trend_BM:mixedType +
                 generalization + time_A:generalization + level_AB:generalization + 
                 trend_AB:generalization + level_BM:generalization + trend_BM:generalization,
               data = wp_data_mlm,
               method = "REML",
               random = ~ level_AB | study/cluster/case,
               correlation = corAR1(value = 0.2, form = ~ session | study/cluster/case),
               weights = varIdent(form = ~ 1 | phase),
               control=list(maxIter=100,msMaxIter=100,tolerance=1e-3,
                            opt="optim",optimMethod="BFGS"))
```
...
Model.4.crve.mixed <- cbind(Model.4.crve, Model.4.d) %>%
  mutate_if(is.numeric, ~round(., 4)) %>% tibble() %>%
  rename(Estimate = beta,
         "t-stat" = tstat,
         "d.f. (Satt)" = df_Satt,
         "p-val (Satt)" = p_Satt)
...
```{r, comment = NA, class.source = 'fold-show'}
sqrt(diag(varcomp_vcov(Model.4))) # Standard errors of variances 
```
...
acf_Model.4.value <- tidy(acf_Model.4$ACF) %>%
  mutate(acf = round(x, 2)) %>%
  select(acf)
acf_Model.4.tb <- tableGrob(acf_Model.4.value[1:10,], rows = NULL, theme = ttheme_default(base_size = 10))
grid.arrange(acf_Model.4.plot, acf_Model.4.tb, nrow = 1, widths = c(0.85, 0.15))
```
